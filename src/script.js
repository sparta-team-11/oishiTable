        ...
        success: function (authObj) {
            Kakao.API.request({
                ...
                success: function (res) {
                    ...
                    sendKakaoLoginToBackend(kakaoEmail);
                },
                fail: function (error) {
...
async function sendKakaoLoginToBackend(email) {
    try {
        const response = await fetch('/api/auth/kakao/callback?code=' + email, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function searchStores() {
    ...
    const searchResults = [
        {id: 'store1', name: '검색 결과 1', bookmarked: false},
        {id: 'store2', name: '검색 결과 2', bookmarked: true}
    ];
...
function displayStores(stores) {
    ...
    stores.forEach(store => {
        ...
        storeItem.innerHTML = `
            <h2>${store.name}</h2>
            <button class="bookmark-button ${store.bookmarked ? 'active' : ''}"
                    onclick="toggleBookmark('${store.id}', this)">
